...
<template>
    <div class="margin-top-30">
        ...
        <!-- 题库选择窗口 -->
        <Modal draggable :mask-closable="false" @on-cancel="handlecancel" ok-text="确定" v-model="subjectmodal" :styles="{top: '20px'}"
            width="900" @on-ok="handleSaveSubject">
            <subjectcmpts :SubjecSubClass="SubjecSubClass" ref="subjectcmpts"></subjectcmpts>
        </Modal>
        <!-- 组卷方式窗口 -->
        <Modal fullscreen :mask-closable="false" @on-cancel="handlecancelGroup" ok-text="确定" v-model="rdmmodal" width="600" @on-ok="handleSaveSubjectRule">
        ...
        watch: {
            ...
            'subjectTitleSel': {
                ...
                handler (val, oldVal) {
                    ...
                    if (parseInt(this.examrqParam.AssemblyType) !== 10) {
        ...
        methods: {
            ...
            setPaperSubject (type, dt, row) {
                let examrqParam = { // 需要传入后台的参数
                    ...
                    AssemblyType: row.AssemblyType, // 组卷方式
                    PaperMode: row.PaperMode, // 试卷类型
                    SubjectData: [] // 题库数据
                };
                this.examrqParam = examrqParam;
                if (type === 10) {
                        ...
                        switch (parseInt(dt[i].SubjecSubClass)) {
                            case 11:
                                for (let j = 0; j < dt[i].subjectlist.length; j++) {
                                    ...
                                    if (!util.isArrayFn(danx.SelectionOption)) {
